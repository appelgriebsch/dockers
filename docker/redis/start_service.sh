...
if ! [ -z "$REDIS_DBDIR" ]; then
  ...
  cp /etc/redis.conf $REDIS_DBDIR/redis.conf && \
  dbpath=`echo $REDIS_DBDIR | sed -e 's/\//\\\\\//g'`
  sed -i "s/^\(dir .*\)$/# \1\ndir $dbpath/" $REDIS_DBDIR/redis.conf 
fi
...
if ! [ "$REDIS_MASTER" == "''" ]; then
  ...
  sed -i "s/^\(# slaveof .*\)$/\1\nslaveof $REDIS_MASTER_IP $REDIS_MASTER_PORT/" $REDIS_DBDIR/redis.conf 
fi

echo Checking database log level...$REDIS_LOGLVL
if ! [ -z "$REDIS_LOGLVL" ]; then
  echo Setting up log level $REDIS_LOGLVL.
  sed -i "s/^\(loglevel .*\)$/# \1\nloglevel $REDIS_LOGLVL/" $REDIS_DBDIR/redis.conf
fi

/usr/bin/redis-server $REDIS_DBDIR/redis.conf
