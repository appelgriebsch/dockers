...
function setupInstanceNetworking() {
  ...
  if [ "$PGSQL_BIND" == "127.0.0.1" ]; then
    ...
    replaceInFile $PGSQL_DBDIR/$HOSTNAME/postgresql.conf "s/^\(#unix_socket_directories .*\)$" "\1\nunix_socket_directories = '$PGSQL_DBDIR/$HOSTNAME'"
    replaceInFile $PGSQL_DBDIR/$HOSTNAME/postgresql.conf "s/^\(#unix_socket_group .*\)$" "\1\nunix_socket_group = 'postgres'"
    replaceInFile $PGSQL_DBDIR/$HOSTNAME/postgresql.conf "s/^\(#unix_socket_permissions .*\)$" "\1\nunix_socket_permissions = '0755'"
    addToFile $PGSQL_DBDIR/$HOSTNAME/pg_hba.conf "local all all md5"
  else
    echo Enabling network binding on IP $PGSQL_BIND.
    replaceInFile $PGSQL_DBDIR/$HOSTNAME/postgresql.conf "s/^\(#listen_addresses.*\)$/listen_addresses = '$PGSQL_BIND'\n\1/"
